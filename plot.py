...
def plot_highest_curves(data_lists, names, filepath="./my_plot.png", x_label="X", y_label="Y", color="-r", kernel_size=50, grid=True):
    """Plot a graph using matplotlib
    ...
    for i in range(len(data_lists)):
        ...
        #print("arange: " + str(np.arange(tot_data)[lower_boundary:upper_boundary]))
        #print("Convolved: " + str(np.arange(tot_data).shape))
        ax.plot(np.arange(tot_data)[lower_boundary:upper_boundary], data_convolved_array, label = "Algorithm: " + names[i], alpha=1.0)  # Convolved plot
    ax.legend()
    fig.savefig(filepath)
    fig.clear()
    plt.close(fig)
